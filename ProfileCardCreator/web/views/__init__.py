from .auth_views import RegisterView, LoginView, LogoutView, IndexView
from .task import TasksListView, CreateTaskView
from .category import CategoryListView, CategoryCreateView
from .fields_of_work import FieldOfWorkListView, FieldOfWorkCreateView
from .subtask import SubtaskCreateView, SubtaskListView
from .item import ItemCreateView, ItemDetailView, ItemListView
